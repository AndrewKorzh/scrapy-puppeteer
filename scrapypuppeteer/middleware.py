...
class PuppeteerServiceCustomDownloaderMiddleware:
...
'''
        print("\n\n\n")
        print(f"puppeteer_request: {puppeteer_request} - {type(puppeteer_request)}")

        v = vars(puppeteer_request)
        for ell in list(v.items()):
            print(ell)

        print(f"response: {response} - {type(response)}")

        v2 = vars(response)
        for ell in list(v2.items()):
            print(ell)
